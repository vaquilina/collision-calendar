...
type SpaceEntry = { id: number; name: string; calendarid: number; created_at: string };
type SpaceAccessData = { spaceid: number; userid: number; permissions: AccessPermissions; created_at: string };
...
Deno.test('DB: user accessors', async (t) => {
  ...
  await t.step('accessor: get all user logins with space access', () => {
    const mock_user_data: UserData[] = [];
    ...
    db.execute(
      `
        ...
        VALUES ('${mock_space_data.name}', ${mock_space_data.calendarid}, '${mock_space_data.created_at}')
      `,
    );
    ...
    // create space access for a subset of the users
    ...
    assertExists(space_access_entries, 'inserted space access entries not found');
    assertEquals(space_access_entries.length, number_of_cal_access);

    // counts
    const users_with_permissions = {
      [access_permissions_values[0]]: space_access_entries.filter((cae) =>
        cae.permissions === access_permissions_values[0]
      ).length,
      [access_permissions_values[1]]:
        space_access_entries.filter((cae) => cae.permissions === access_permissions_values[1]).length,
      [access_permissions_values[2]]:
        space_access_entries.filter((cae) => cae.permissions === access_permissions_values[2]).length,
    };
    ...
    do {
      index_perms_to_test_for = randomIntegerBetween(0, 2);
      results = getAllUsersWithSpaceAccess(
        space_entry.id,
        access_permissions_values[index_perms_to_test_for],
        db,
      );
